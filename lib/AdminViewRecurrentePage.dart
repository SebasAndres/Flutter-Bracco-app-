...
class AdminViewRecurrentePage extends StatefulWidget {
  @override
  _recnoadmin createState() => _recnoadmin();
}

class _recnoadmin extends State<AdminViewRecurrentePage> {
    ...
    setState(()=>{
      databaseReference.child("Recurrentes").once().then((DataSnapshot snapshot){
        ...
        for (int z=0; z<size; z++){
          ...
          if (Dif < D+1 && pendiente["Noti"] == 0) { // Si hay una distancia entre las fechas menor igual a D días
            PENDIENTES.add(z);
  ...
  Widget devContainer (int index){
    return InkWell(
      ...
      onLongPress: (){
        ...
        databaseReference.child("Recurrentes").child(PENDIENTES[index].toString()).child("Noti").set(1);
